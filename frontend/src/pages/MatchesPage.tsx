...
import { itemsApi, type ItemMatch } from '../api/items.api'
import { CONDITION_LABELS } from '../types/item'
import { Sparkles, MapPin, Repeat } from 'lucide-react'
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Вещи, которые подходят под желаемые вами обмены
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-slate-400 dark:text-slate-500">
          ...
          <p className="text-lg font-semibold text-slate-600 dark:text-slate-300">Совпадений пока нет</p>
          <p className="text-sm mt-1">
            Укажите «желаемые предметы» у своих вещей, чтобы мы могли подобрать обмены.
          </p>
          <Link to="/my-items" className="text-indigo-600 dark:text-indigo-400 hover:underline text-sm mt-2 inline-block">
            Перейти к моим вещам
          </Link>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {matches.map(({ item, mutual, matchedOn }) => (
            <Link
              key={item.id}
              to={`/items/${item.id}`}
            ...
            >
              <div className="aspect-square bg-slate-50 dark:bg-slate-900 overflow-hidden relative border-b border-slate-100 dark:border-slate-700">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-1.5 items-start">
                  ...
                  {mutual && (
                    <span className="flex items-center gap-1 bg-emerald-600/90 backdrop-blur-sm text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
                      <Repeat size={11} /> Взаимный интерес
                    </span>
                  )}
                </div>
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {matchedOn.slice(0, 3).map((kw) => (
                    <span key={kw} className="text-xs bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-full">
                      {kw}
                    </span>
                  ))}
                </div>
